...
1. Run a full live-Azure verification of both `VM_CREATE` and
...
5. Keep `.memory/` updated as part of "definition of done" for every future
   feature (standing process requirement — see `CLAUDE.md` → "Session Memory
   Protocol").
6. Run `graphify update .` after any further code changes to keep the
